...
namespace SimplexLab.WeaveMaze.TApplication
{
    internal class WeaveMazeRenderer
    {
        ...
        private Color wallColor = Colors.Black;
        ...
        public void Draw(DrawingContext context)
        {
            ...
            // 绘制背景
            context.DrawRectangle(new SolidColorBrush(backgroundColor), null,
            ...
            var wallPen = new Pen(new SolidColorBrush(wallColor), lineW)
            {
                LineCap = roundedCorners ? PenLineCap.Round : PenLineCap.Square,
                LineJoin = PenLineJoin.Round
            };

            DrawWallPaths(context, wallPen, cells, mazeHeight, mazeWidth, cellSize, d0, d1, dm, r0);
        }

        #region 墙壁绘制

        private void DrawWallPaths(DrawingContext context,
                                   Pen pen,
                                   SquareCell[][] cells,
                                   int mazeHeight,
                                   int mazeWidth,
                                   ...
                                   float r0)
        {
            ...
            for (int i = 0; i < mazeHeight; i++)
            {
                ...
                for (int j = 0; j < mazeWidth; j++)
                {
                    ...
                    {
                        DrawWallNorthSouthOver(geometry, ox, oy, cellSize, d0, d1);
                    }
                    else if (cell.Upper.East != null)
                    {
                        DrawWallEastWestOver(geometry, ox, oy, cellSize, d0, d1);
                    }
                    else
                    {
                        ...
                        DrawWallFlat(geometry, ox, oy, cellSize, d0, d1, dm, r0, value);
                    }
                }
            }
        ...
        private void DrawWallNorthSouthOver(StreamGeometry path, float ox, float oy, float cellSize, float d0, float d1)
        {
        ...
        private void DrawWallEastWestOver(StreamGeometry path, float ox, float oy, float cellSize, float d0, float d1)
        {
            pathBuilder.MoveTo(path, ox, oy + d0);
            pathBuilder.LineTo(path, ox + cellSize, oy + d0);
            pathBuilder.MoveTo(path, ox, oy + d1);
        ...
        private void DrawWallFlat(StreamGeometry path,
                                  float ox,
                                  float oy,
                                  float cellSize,
                                  float d0,
